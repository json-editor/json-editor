...
this.property_order=Object.keys(this.editors),this.property_order=this.property_order.sort(function(a,b){var d=c.editors[a].schema.propertyOrder,e=c.editors[b].schema.propertyOrder;return"number"!=typeof d&&(d=1e3),"number"!=typeof e&&(e=1e3),d-e});var f,g="categories"===this.format;if("grid"===this.format){var h=[];
...
// Layout the form
f=document.createElement("div"),a=0;a<h.length;a++){var k=this.theme.getGridRow();for(f.appendChild(k),b=0;b<h[a].editors.length;b++){var l=h[a].editors[b].key,m=this.editors[l];m.options.hidden?m.container.style.display="none":this.theme.setGridColumnSize(m.container,h[a].editors[b].width),k.appendChild(m.container)}}}else{if(f=document.createElement("div"),g){
//A container for properties not object nor arrays
var n=document.createElement("div"),o=this.theme.getTopTabHolder(),p=this.theme.getTabContentHolder(o);
//Erase old panes
...
f?(e.appendChild(g),p.appendChild(e),o.firstChild.appendChild(d.tab)):(n.appendChild(g),
...
//Add "Basic" tab
o.firstChild.insertBefore(d.tab,o.firstChild.firstChild),
//Update the basicPane
d.basicPane=e):(
//Append pane for simple properties
e.appendChild(n),p.appendChild(e),
//Add "Basic" tab
o.firstChild.appendChild(d.tab),
//Update the basicPane
...
return q.removeChild(q.firstChild),q.appendChild(o),this.tabPanesContainer=p,this.tabs_holder=o,void(this.tabs_holder.firstChild.firstChild&&e(this.tabs_holder.firstChild.firstChild,"click"))}d(this.property_order,function(a,b){var d=c.editors[b];if(!d.property_removed){var e=c.theme.getGridRow();f.appendChild(e),d.options.hidden?d.container.style.display="none":c.theme.setGridColumnSize(d.container,12),e.appendChild(d.container)}})}
...
addTab:function(a){var b=this,c=b.rows[a].schema&&("object"===b.rows[a].schema.type||"array"===b.rows[a].schema.type);b.tabs_holder&&(b.rows[a].tab_text=document.createElement("span"),c?b.rows[a].tab_text.textContent=b.rows[a].getHeaderText():b.rows[a].tab_text.textContent="undefined"==typeof b.schema.basicCategoryTitle?"Basic":b.schema.basicCategoryTitle,b.rows[a].tab=b.theme.getTopTab(b.rows[a].tab_text),b.rows[a].tab.addEventListener("click",function(c){b.active_tab=b.rows[a].tab,b.refreshTabs(),c.preventDefault(),c.stopPropagation()}))},addRow:function(a,b,c){var d=this,e=this.rows.length,f="object"===a.schema.type||"array"===a.schema.type;
//Add a row
d.rows[e]=a,
//rowPane stores the editor corresponding pane to set the display style when refreshing Tabs
d.rows[e].rowPane=c,f?(d.addTab(e),d.theme.addTab(b,d.rows[e].tab)):
...
d.basicTab=e,d.basicPane=c,d.theme.addTab(b,d.rows[e].tab)):(
...
f.tab&&f.rowPane&&f.rowPane.parentNode&&(c&&f.tab==b.rows[b.basicTab].tab&&e||(a?f.tab_text.textContent=f.getHeaderText():(
//All rows of simple properties point to the same tab, so refresh just once
c&&f.tab==b.rows[b.basicTab].tab&&(e=!0),f.tab===b.active_tab?(b.theme.markTabActive(f.tab),f.rowPane.style.display=""):(b.theme.markTabInactive(f.tab),f.rowPane.style.display="none"))))})},build:function(){var a=this,b="categories"===this.format;
// If the object should be rendered as a table row
...
this.getMax()&&this.getMax()<=this.rows.length||this.hide_add_button?this.add_row_button.style.display="none":(this.add_row_button.style.display="",f=!0),!this.collapsed&&f?this.controls.style.display="inline-block":this.controls.style.display="none"}},addRow:function(a,b){var c=this,e=this.rows.length;c.rows[e]=this.getElementEditor(e),c.row_cache[e]=c.rows[e],c.tabs_holder&&(c.rows[e].tab_text=document.createElement("span"),c.rows[e].tab_text.textContent=c.rows[e].getHeaderText(),"tabs-top"===c.schema.format?c.rows[e].tab=c.theme.getTopTab(c.rows[e].tab_text):c.rows[e].tab=c.theme.getTab(c.rows[e].tab_text),c.rows[e].tab.addEventListener("click",function(a){c.active_tab=c.rows[e].tab,c.refreshTabs(),a.preventDefault(),a.stopPropagation()}),c.theme.addTab(c.tabs_holder,c.rows[e].tab));var f=c.rows[e].title_controls||c.rows[e].array_controls;
...
f.defaults.themes.foundation3=f.defaults.themes.foundation.extend({getHeaderButtonHolder:function(){var a=this._super();return a.style.fontSize=".6em",a},getFormInputLabel:function(a){var b=this._super(a);return b.style.fontWeight="bold",b},getTabHolder:function(){var a=document.createElement("div");return a.className="row",a.innerHTML="<dl class='tabs vertical two columns'></dl><div class='tabs-content ten columns'></div>",a},getTopTabHolder:function(){var a=document.createElement("div");return a.className="row",a.innerHTML="<dl class='tabs horizontal' style='padding-left: 10px; margin-left: 10px;'></dl><div class='tabs-content ten columns'></div>",a},setGridColumnSize:function(a,b){var c=["zero","one","two","three","four","five","six","seven","eight","nine","ten","eleven","twelve"];a.className="columns "+c[b]},getTab:function(a){var b=document.createElement("dd"),c=document.createElement("a");return c.setAttribute("href","#"),c.appendChild(a),b.appendChild(c),b},getTopTab:function(a){var b=document.createElement("dd"),c=document.createElement("a");return c.setAttribute("href","#"),c.appendChild(a),b.appendChild(c),b},getTabContentHolder:function(a){return a.children[1]},getTabContent:function(){var a=document.createElement("div");return a.className="content active",a.style.paddingLeft="5px",a},markTabActive:function(a){a.className=a.className.replace(/\s?active/g,""),a.className+=" active"},markTabInactive:function(a){a.className=a.className.replace(/\s?active/g,"")},addTab:function(a,b){a.children[0].appendChild(b)}}),
