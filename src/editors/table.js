  ...
  refreshRowButtons () {
    ...
    this.rows.forEach((editor, i) => {
      if (editor.delete_button) {
        ...
        this.setDisable(editor.delete_button, display)
      ...
      if (editor.copy_button) {
        ...
        this.setDisable(editor.copy_button, display)
        needRowButtons.push(display)
      }
      ...
      if (editor.moveup_button) {
        ...
        const display = i !== 0
        this.setDisable(editor.moveup_button, display)
        needRowButtons.push(display)
      }
    ...
    this.rows.forEach((editor) =>
      this.setDisable(editor.controls_cell, need)
    )
    this.setDisable(this.controls_header_cell, need)

    this.setDisable(this.table, this.value.length)
    ...
    this.setDisable(this.add_row_button, display1)
    ...
    this.setDisable(this.delete_last_row_button, display2)
    ...
    this.setDisable(this.remove_all_rows_button, display3)

    const controlsNeeded = display1 || display2 || display3
    this.setDisable(this.controls, controlsNeeded)
  }
  ...
  addControls () {
    ...
    if (this.title_controls) {
      ...
      this.toggle_button.addEventListener('click', e => {
        ...
        this.setDisable(this.panel, this.collapsed)
        if (this.collapsed) {
          this.collapsed = false
